...
if [[ "$KIND" != "qa" && "$KIND" != "tw" && "$KIND" != "rf" ]]; then
  echo "Usage: $0 {qa|tw|rf} [task-id]" >&2
  exit 2
fi
...
cd "$PRODUCT_ROOT"
python3 -m benchmarks.swe_atlas.prepare_task "${PREPARE_ARGS[@]}"

export PYTHONPATH="$PRODUCT_ROOT${PYTHONPATH:+:$PYTHONPATH}"
"$HARBOR_BIN" run \
  -c "$SCRIPT_DIR/config/$KIND.yaml" \
  --job-name "krater-pro-kimi-k3-swe-atlas-$KIND-smoke" \
  -y
