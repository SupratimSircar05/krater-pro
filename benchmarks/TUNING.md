...
| Security and Authentication | KC-031–040 | OAuth/OIDC, JWT, WebAuthn, capability sandboxes, webhook signing, redaction, safe extraction, policy engines, recovery, and SSRF/DNS rebinding | **Partial, with useful guardrails.** Workspace confinement, secret-path blocking, reduced command environment, loopback-only GUI binding, approvals, and destructive-command rejection address part of the threat surface. | Command policy is regex-based rather than parser- and capability-based; tool output is not a hardened untrusted-data channel; there is no OS sandbox or formal authorization engine. Krater Pro currently exposes an API-key handoff, not an official Krater OAuth flow, and intentionally does not scrape browser cookies or private session tokens. |
...
| Agent Safety and Repository Operations | KC-091–100 | Dirty trees, shell policy, secret-safe indexing, TOCTOU, atomic refactors, prompt injection, worktrees, scoped approval, completion evidence, and bounded autonomy | **Strongest relative coverage, still partial.** Dirty-tree inspection, workspace/symlink checks, protected secrets, mutation approvals, destructive-command blocks, reduced environments, output bounds, cache invalidation, and maximum steps directly address several tasks. The benchmark runner adds isolated copies and optional external checks. | No atomic multi-file transaction or rollback, structured shell parser, OS sandbox, prompt-injection isolation, worktree planner, durable audit log, capability-scoped approval token, or evidence-backed completion gate. There are TOCTOU windows between path validation and filesystem mutation. |
...
5. **There is no execution sandbox.** A permitted command runs with the user's
   workspace permissions. Environment filtering protects known credentials but
   does not constrain filesystem access, network access, CPU, memory, child
   processes, or access to credentials available through other system
   facilities.
